...
export default function VehiclesComponent({ data }: any) {
    const vehicle = data[0];
    const [checked, setChecked] = useState(vehicle?.status);
    ...
    return (
        <>
            <div className="flex justify-between items-center my-2">
                <h1 className="text-2xl">{vehicle?.type} package</h1>
                <div className="flex justify-center items-center gap-3">
                    <p className="mt-1 text-[#929292]">{checked ? 'Packages available' : 'Packages Unavailable'}</p>
                    <p>
                        <Switch onChange={changeToogle} />
                    </p>
                </div>
            </div>
            <div className="flex flex-col md:flex-row  gap-4">
                {vehicle?.plan?.map((plan: any) => {
                    return (
                        <div
                            key={plan?._id}
